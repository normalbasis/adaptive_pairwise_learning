...
def plot_iter(data, loss, method, i):
    options = dict()
    options['n_proc'] = 12
    options['rec_log'] = .5
    options['rec'] = 50
    ...
    options['eta_list'] = [10.]
    options['beta_list'] = [10.] # only one beta
    options['n_repeat'] = 5
    options['n_split'] = 5
    ...
    if method == 'fifo' or method == 'fifoada':
        options['n_pass'] = 30
    ...
    filename = str(i) + '_' + data + '_' + method_name + '_' + loss
...
if __name__ == '__main__':
    ...
    plot_iter(args.data, args.loss, args.method, args.index)