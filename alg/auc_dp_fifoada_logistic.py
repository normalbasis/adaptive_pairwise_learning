...
def auc_fifoada_stage_logistic(stage, w_init, x_tr, y_tr, options):
    n_tr = len(y_tr)
    dim = options['dim']
    ...
    beta = options['beta']
    # theta = options['theta'] #initial theta
    theta = 0.9
    m = np.zeros(dim) # initilize m
    v = np.zeros(dim) # initilize v	
    t = 0
    time_sum = 0.
    while t < n_iter:
        start = timeit.default_timer()
        ...
        if y_t * y_t_1 < 0:
            ...
            #     eta_t = eta_t / 10
            # gradient step
            m = theta * m + (1-theta)*gd
            v = v + np.inner(gd, gd)
            w_t = w_t - eta_t * m/np.sqrt(v)			
            # w_t = w_t - eta_t * gd
            # w_t = w_t_1 - eta_t * gd
            # projection
...
def auc_dp_fifoada_logistic(x_tr, y_tr, x_te, y_te, options):
    # get
    n_tr = options['n_tr']
    dim = options['dim']
    ...
    while stage < len(stages):
        ...
        # warm start
        w_t, time_t = auc_fifoada_stage_logistic(stage, w_priv, x_tr_stage, y_tr_stage, options)
        # start timer
        start = timeit.default_timer()
        # calibrate noise
...
if __name__ == '__main__':
    ...
    for i, (tr_idx, te_idx) in enumerate(k_fold.split(x)):
        ...
        auc_t, time_t = auc_dp_fifoada_logistic(x_tr, y_tr, x_te, y_te, options)
        time_rec[i] = time_t
        auc_rec[i] = auc_t
    print('----------------------')
