...
from alg import auc_fifoada_hinge
from alg import auc_fifoada_square
from alg import auc_fifo_square
from alg import auc_fifo_loglink
from alg import auc_fifoada_loglink
...
# from alg.auc_dp_fifoada_logistic import auc_fifoada_stage_logistic
# use fifoada_logistic w/o stage 
import numpy as np
...
def auc_fix_para(x, y, loss, method, options, k):
    ...
    for i, (tr_idx, te_idx) in enumerate(k_fold.split(x)):
        ...
        if loss == 'hinge':
            ...
            elif method == 'auc_fifoada':
                auc_t, time_t = auc_fifoada_hinge(x_tr, y_tr, x_te, y_te, options)
            else:
            ...
            if method == 'auc_fifo':
                ...
                auc_t, time_t = auc_fifoada_loglink(x_tr, y_tr, x_te, y_te, options)                
            else:
                print('Wrong method name!')
                return
        elif loss == 'logistic':
            if method == 'auc_fifo':
                ...
                auc_t, time_t = auc_fifoada_logistic(x_tr, y_tr, x_te, y_te, options)
            else:
                print('Wrong method name!')
                return
        elif loss == 'square':
            if method == 'auc_spauc':
                ...
                auc_t, time_t = auc_fifoada_square(x_tr, y_tr, x_te, y_te, options)                
            else:
                print('Wrong method name!')
                return
        else:
